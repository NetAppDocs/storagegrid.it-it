...
+
--
Se intendi utilizzare richieste in stile virtual-hosted di S3, dovresti avere link:../admin/configuring-s3-api-endpoint-domain-names.html["Nomi di dominio degli endpoint S3 configurati"]. Si consiglia di utilizzare richieste in stile virtual-hosted.
...
--
Configurare le connessioni di federazione di griglie:: Se vuoi consentire al tenant S3 di sincronizzare i dati dell'account e replicare gli oggetti del bucket in un'altra griglia utilizzando una connessione di federazione di griglia, conferma quanto segue prima di avviare il setup wizard:
+
--
