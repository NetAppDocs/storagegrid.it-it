...
summary: 'Prima di poter utilizzare un server di gestione delle chiavi (KMS) per proteggere i dati di StorageGRID sui nodi dell"appliance, devi completare due attività di configurazione: impostare uno o più server KMS e abilitare la crittografia dei nodi per i nodi dell"appliance. Quando queste due attività di configurazione sono complete, il processo di gestione delle chiavi avviene automaticamente.' 
---
