...
summary: 'Prima di poter utilizzare un server di gestione delle chiavi (KMS) per proteggere i dati di StorageGRID sui nodi dell"appliance, devi configurare uno o più server KMS e abilitare la crittografia dei nodi per i nodi dell"appliance. Quando queste due operazioni di configurazione sono complete, il processo di gestione delle chiavi avviene automaticamente.' 
---
...
image:kms_configuration_overview.png["Flusso di lavoro di configurazione KMS, descritto nel testo seguente"]
