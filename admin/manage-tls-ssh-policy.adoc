...
+
image:securitysettings_tls_ssh_policies_current.png["Policy TLS e SSH"]
...
.FIPS rigoroso
--
Utilizza la policy FIPS strict se hai bisogno della certificazione Common Criteria e devi usare il NetApp Cryptographic Security Module (NCSM) 3.1.2 o il modulo NetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1 per le connessioni client esterne agli endpoint del bilanciatore di carico, a Tenant Manager e a Grid Manager. Usare questa policy potrebbe ridurre le prestazioni.

NCSM 3.1.2 e il modulo NetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1 vengono utilizzati nelle seguenti operazioni:
...
+
Per ulteriori informazioni, consulta il Programma di convalida degli algoritmi crittografici del NIST link:https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/details?validation=40426["Certificato #A7816"^].
...
+
*Nota*: Questo modulo non è stato sottoposto a certificazione.



+
Per ulteriori informazioni, consulta il Programma di convalida degli algoritmi crittografici del NIST link:https://csrc.nist.gov/projects/cryptographic-module-validation-program/entropy-validations/certificate/223["Certificato di entropia n. E223"^].
...
Facoltativamente, se il tuo StorageGRID ha requisiti di crittografia FIPS 140, abilita la funzionalità della modalità FIPS per utilizzare NCSM 3.1.2 e il modulo NetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1:
...
+
image:securitysettings_custom_security_policy_copy.png["copia di un criterio esistente per creare un criterio personalizzato"]
