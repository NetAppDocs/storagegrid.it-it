...
summary: La replica cross-grid è la replica automatica di oggetti tra bucket S3 selezionati in due sistemi StorageGRID connessi tramite una connessione di federazione grid. La sincronizzazione degli account è necessaria per la replica cross-grid. 
---
...
[role="lead"]
La replica cross-grid è la replica automatica di oggetti tra bucket S3 selezionati in due sistemi StorageGRID connessi in una link:grid-federation-overview.html["connessione a federazione di griglie"]. link:grid-federation-what-is-account-sync.html["Sincronizzazione account"] è richiesta per la replica cross-grid.
...
image:grid_federation_cgr_workflow.png["Workflow di replica cross-grid"]
...
Se un tenant account ha l'autorizzazione *Usa connessione di federazione griglia* per utilizzare uno o più link:grid-federation-overview.html["connessioni a federazione di griglie"], un tenant con autorizzazione di accesso Root può creare bucket ("source" e "destination" bucket) nei corrispondenti tenant account su ogni griglia. Questi bucket:

* Possono avere nomi diversi l'uno dall'altro
* Può avere regioni diverse
* È necessario attivare la versione
* Devi avere una delle seguenti configurazioni di S3 Object Lock:
+
** origine: *disabilitato*, destinazione: *disabilitato*
** origine: *abilitata*, destinazione: *abilitata*
** origine: *disabilitata*, destinazione: *abilitata*




Dopo aver creato entrambi i bucket, puoi configurare la replica tra griglie per uno o entrambi i bucket.
...
image:grid_federation_cross_grid_replication_one_direction.png["immagine che mostra la connessione a federazione di griglie in una direzione"]
...
image:grid_federation_cross_grid_replication.png["immagine che mostra la replica in una direzione rispetto alla replica in entrambe le direzioni"]
...
image:grid_federation_cross_grid_replication_delete.png["immagine che mostra l'eliminazione del client replicato su entrambe le griglie"]
...
CAUTION: Puoi abilitare il blocco degli oggetti S3 sui bucket esistenti, il che potrebbe portare a una configurazione errata in seguito.


NOTE: Quando S3 Object Lock è abilitato sul bucket di origine, assicurati che sia abilitato anche sul bucket di destinazione. Se S3 Object Lock è disabilitato sul bucket di destinazione, si verifica un errore durante la replica. Consulta link:../admin/grid-federation-troubleshoot.html#cross-grid-replication-alerts-and-errors["Avvisi ed errori di replica cross-grid"].
