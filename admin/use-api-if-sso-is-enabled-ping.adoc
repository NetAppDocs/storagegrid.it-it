...
[role="lead"]
Se hailink:../admin/how-sso-works.html["SSO (Single Sign-on) configurato e abilitato"] e utilizzi PingFederate come provider SSO, devi inviare una serie di richieste API per ottenere un token di autenticazione valido per l'API Grid Management o l'API Tenant Management.
