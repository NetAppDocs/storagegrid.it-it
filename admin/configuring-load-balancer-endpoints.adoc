...
Se stai configurando gli endpoint del bilanciatore di carico per un link:../admin/configure-global-namespace.html["spazio dei nomi globale"], usa la stessa porta per ogni endpoint.
...
+
image:load_balancer_endpoint_details.png["Dettagli dell'endpoint del bilanciamento del carico"]
...
.. Seleziona l'icona di modifica image:icon_edit_tm.png["Icona Edit (Modifica)"].
.. Inserire il nuovo nome.
.. Selezionare *Salva*.
