...
summary: 'Potresti non riuscire ad accedere a Grid Manager se il sistema di single sign-on (SSO) non funziona. In questo caso, puoi disabilitare e riabilitare temporaneamente il single sign-on per un Admin Node. Per disabilitare e poi riabilitare il single sign-on, devi accedere alla shell dei comandi del nodo.' 
---
...
. A seconda della configurazione SSO, esegui uno dei seguenti comandi:
+
[role="tabbed-block"]
====
.SAML
