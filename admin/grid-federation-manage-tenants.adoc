...
+
image:grid-federation-tenant-detail.png["Scheda Grid Federation (federazione griglia) dalla pagina dei dettagli del tenant"]
...
image:grid-federation-remove-permission.png["procedura per la rimozione della connessione a federazione di griglie"]
...
+
image:grid-federation-remove-permission-error.png["viene visualizzato un messaggio di errore se il tenant ha cgr attivato per un bucket"]
