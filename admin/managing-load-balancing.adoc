...
Se vuoi utilizzare richieste in stile virtual-hosted di S3, il certificato deve includere anche una voce *Nome alternativo* per ciascuna link:../admin/configuring-s3-api-endpoint-domain-names.html["Nome di dominio dell'endpoint S3"] che hai configurato, compresi eventuali nomi jolly. Ad esempio:
