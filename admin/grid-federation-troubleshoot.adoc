...
summary: Potresti dover risolvere problemi relativi ad avvisi ed errori riguardanti le connessioni di federazione della griglia, la sincronizzazione degli account e la replica tra griglie. 
---
= Risoluzione dei problemi degli errori di federazione della griglia in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Potresti dover risolvere problemi relativi ad avvisi ed errori riguardanti le connessioni di federazione della griglia, la sincronizzazione degli account e la replica tra griglie.
...
[[account-sync-errors]]
== Errori di sincronizzazione dell'account



[[cant-sign-in-to-a-synced-tenant-account]]
=== Impossibile accedere a un account tenant sincronizzato

.Problema
Non puoi accedere a un account tenant sincronizzato. Il messaggio di errore nella pagina di accesso di Tenant Manager è "Le credenziali per questo account non erano valide. Riprova."

.Dettagli
La password che imposti per l'utente root locale del tenant e per gli altri utenti locali viene sincronizzata automaticamente. Tuttavia, potresti dover cambiare la password per l'utente root locale del tenant sulla griglia di destinazione prima di poter accedere all'account del tenant su quella griglia.
...
=== Tenant creato senza sincronizzazione

.Problema
Vedi il messaggio "Tenant creato senza sincronizzazione" dopo aver creato un nuovo tenant con l'autorizzazione *Use grid federation connection*.
...
. Individua il tenant account che non è stato sincronizzato.
. Selezionare il nome del tenant per visualizzare la pagina dei dettagli.
. Seleziona *Riprova la sincronizzazione*. Se l'errore è stato risolto, il tenant account verrà ora sincronizzato con l'altra grid.
...
image:grid_federation_last_error.png["Schermata che mostra un messaggio nella colonna Last error (ultimo errore) di una connessione a federazione di griglie"]
...
. Esegui tutte le azioni raccomandate.
+
Ad esempio, se il versioning è stato sospeso sul bucket di destinazione per la replica cross-grid, riattiva il versioning per quel bucket. Oppure, se S3 Object Lock è stato disabilitato sul bucket di destinazione per la replica cross-grid, abilita S3 Object Lock per quel bucket.
