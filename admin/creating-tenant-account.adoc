...
* Se vuoi consentire a un tenant S3 di sincronizzare i dati dell'account e replicare gli oggetti del bucket in un'altra griglia utilizzando una connessione di federazione di griglia:
+
...
Connessione federazione griglia
 a| 
Il tenant può utilizzare le connessioni di federazione della griglia, che:

** Fa sì che questo account tenant e tutti i gruppi tenant, utenti e chiavi di accesso S3 vengano sincronizzati tra questa griglia e le altre griglie nelle connessioni selezionate.
+

TIP: Se necessario per motivi di sicurezza o conformità, puoi deselezionare *Abilita la sincronizzazione della chiave di accesso*.
...
. Se hai selezionato l'autorizzazione *Usa connessione di federazione griglia* per il tenant, verifica che un tenant identico sia stato replicato sull'altra griglia nella connessione. I tenant su entrambe le griglie avranno lo stesso ID account a 20 cifre, nome, descrizione, quota e autorizzazioni.
+

NOTE: Se visualizzi un messaggio di errore che indica che il tenant account è stato creato su questa grid ma non è stato sincronizzato con le altre grid, fai riferimento alle istruzioni in link:grid-federation-troubleshoot.html["Risolvere i problemi relativi agli errori di federazione della griglia"].
...
Segui le istruzioni in link:../tenant/index.html["Utilizzare un account tenant"] per gestire gruppi di tenant e utenti, chiavi di accesso S3, bucket, servizi della piattaforma, sincronizzazione degli account e replica cross-grid.
