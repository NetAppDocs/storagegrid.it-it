...
---
= Crea applicazioni aziendali in Entra ID per SAML in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Utilizza Entra ID per creare un'applicazione aziendale per ciascun Admin Node per SAML nel tuo sistema.

.Prima di iniziare
* Hai iniziato a configurare l'accesso Single Sign-On per StorageGRID e hai selezionato *Entra ID* come tipo di SSO.
* Hai link:../admin/configure-sso-saml.html#enter-sandbox-mode["entrato in modalità sandbox"] in Grid Manager.
...
== Passaggio 2: Crea applicazioni aziendali e salva la configurazione SSO di StorageGRID
...
== Passaggio 3: Scarica i metadati SAML per ogni Admin Node
...
== Passaggio 4: Carica i metadati SAML in ogni applicazione aziendale
...
. link:../admin/configure-sso-saml.html#test-sso["Testare ogni applicazione"].
