...
---
= Gestisci i costi dei collegamenti in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* I costi di collegamento vengono utilizzati dal servizio Load Balancer sui nodi di amministrazione e sui nodi gateway per indirizzare le connessioni dei client. Consulta link:../admin/managing-load-balancing.html["Considerazioni per il bilanciamento del carico"].


Il diagramma mostra una griglia a tre siti con costi di collegamento configurati tra i siti:

image:link_costs.png["Diagramma concettuale per i costi di collegamento tra i data center"]
...
. Per ogni sito elencato nella colonna Costo sito/link, inserisci un valore di costo da 0 a 100 per gli altri siti.
+
Non puoi modificare il costo del collegamento se l'origine è la stessa della destinazione.

. Scarta o salva le modifiche:
+
** Per annullare le modifiche, seleziona *Cancella modifiche*.
