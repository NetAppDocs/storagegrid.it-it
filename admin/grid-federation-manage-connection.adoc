...
NOTE: Durante la modifica della connessione, le richieste di sincronizzazione dell'account o di replica tra griglie continueranno a utilizzare le impostazioni di connessione esistenti. Devi modificare una connessione alla volta e caricare un file di verifica sulla griglia remota per ciascuna connessione. Le modifiche vengono applicate dopo che sono state caricate sulle griglie remote.
...
image:grid_federation_remove_connection.png["procedura per la rimozione della connessione a federazione di griglie"]
