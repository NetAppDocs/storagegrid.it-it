...
---
= Crea connessioni service provider in PingFederate per SAML in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Utilizza PingFederate per creare una connessione service provider (SP) per ciascun Admin Node per SAML nel tuo sistema. Per velocizzare il processo, importerai i metadati SAML da StorageGRID.
