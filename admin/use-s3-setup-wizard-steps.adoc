...
. Se il banner *FabricPool and S3 setup wizard* appare sulla dashboard, seleziona il collegamento nel banner. Se il banner non appare più, seleziona l'icona di aiuto image:icon_help_menu_bar.png["Icona di aiuto"] dalla barra dell'intestazione in Grid Manager e seleziona *FabricPool and S3 setup wizard*.
...
*Nota:* non sono consentite le porte utilizzate da altri servizi di rete. Vedere link:../network/internal-grid-node-communications.html#storagegrid-internal-ports["Porte interne StorageGRID"] .
...
. Definisci l'accesso root per l'account tenant, in base all'utilizzo o meno da parte del tuo sistema StorageGRIDlink:using-identity-federation.html["federazione delle identità"] ,link:how-sso-works.html["SSO (Single Sign-on)"] , o entrambi.
+
[cols="1a,2a"]
|===
...
** Seleziona l'icona di copia (image:icon_tenant_copy_url.png["icona di copia"]) per copiare l'ID della chiave di accesso e la chiave di accesso segreta negli appunti.
