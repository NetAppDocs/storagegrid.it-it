...
summary: 'Puoi creare interfacce VLAN (Virtual LAN) sugli Admin Node e sui Gateway Node e usarle nei gruppi HA e negli endpoint del load balancer per isolare e partizionare il traffico per sicurezza, flessibilità e prestazioni.' 
---
= Configura le interfacce VLAN in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:vlan-create-parent-interfaces.png["Creazione guidata VLAN interfacce principali"]
...
** Se devi rimuovere un'interfaccia padre, seleziona l'icona del cestino image:icon-trash-can.png["icona del cestino"].
