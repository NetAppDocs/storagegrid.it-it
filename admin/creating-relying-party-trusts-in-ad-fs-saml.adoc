...
---
= Crea trust di relying party in AD FS per SAML in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Devi usare Active Directory Federation Services (AD FS) per creare una relying party trust per ogni Admin Node per SAML nel tuo sistema. Puoi creare relying party trust usando i comandi PowerShell, importando i metadati SAML da StorageGRID o inserendo i dati manualmente.

.Prima di iniziare
* È stato configurato Single Sign-on per StorageGRID ed è stato selezionato *ad FS* come tipo di SSO.
* Hai link:../admin/configure-sso-saml.html["entrato in modalità sandbox"] in Grid Manager.
...
. Al termine, torna a StorageGRID e link:../admin/configure-sso-saml.html#test-sso["testare tutti i trust delle parti affidanti"] per verificare che siano configurati correttamente.
...
. Ripetere questa procedura per configurare un trust per tutti i nodi di amministrazione nel sistema StorageGRID.
. Al termine, torna su StorageGRID e link:../admin/configure-sso-saml.html#test-sso["testare tutti i trust delle parti affidanti"] per confermare che siano configurati correttamente.
