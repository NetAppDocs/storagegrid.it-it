...
summary: 'L"API REST S3 utilizzata da StorageGRID viene aggiornata regolarmente.' 
---
= Modifiche al supporto dell'API REST S3 di StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
L'API REST S3 utilizzata da StorageGRID viene aggiornata regolarmente.
...
* `If-Match`
+
L' `If-Match`intestazione si comporta in modo diverso a seconda della configurazione di StorageGRID. Fornire questa intestazione potrebbe restituire un messaggio di errore (501 Not Implemented). Se l' `If-Match`intestazione viene accettata ma non è funzionale, potrebbe essere visualizzato un avviso con istruzioni su come abilitare il comportamento di errore più restrittivo 501 Not Implemented.

* `If-None-Match`
+
L' `If-None-Match`intestazione si comporta in modo diverso a seconda della configurazione di StorageGRID. Fornire questa intestazione potrebbe restituire un messaggio di errore (501 Not Implemented). Se l' `If-None-Match`intestazione viene accettata ma non è funzionale, potrebbe essere visualizzato un avviso con istruzioni su come abilitare il comportamento di errore più restrittivo 501 Not Implemented.

* Aggiunto il supporto per i valori di checksum precalcolati per le seguenti richieste e intestazioni supportate:
+
** CompleteMultipartUpload: `x-amz-checksum-crc32`, `x-amz-checksum-crc32c`, e `x-amz-checksum-crc64nvme`
** CreateMultipartUpload: `CRC32`, `CRC32C`, e `CRC64NVME` per `x-amz-checksum-algorithm`
** CopyObject: `CRC32`, `CRC32C`, `CRC64NVME`, e `SHA256` per `x-amz-checksum-algorithm`
...
* Aggiunto il supporto per  `x-amz-checksum-type` per CreateMultipartUpload e CompleteMultipartUpload.
* Aggiunto il supporto per la  `s3:x-amz-copy-source`chiave della condizione di policy.
* Consenti di abilitare S3 Object Lock sui bucket esistenti.
...
* Aggiunto il supporto per STS AssumeRole e le policy di sessione. Vedi link:../s3/example-session-policies.html["un esempio di politica di sessione"]. Puoi configurare AssumeRole nei gruppi tenant.
...
11,7
 a| 
* Aggiunto link:../s3/quick-reference-support-for-aws-apis.html["Riferimento rapido: Richieste API S3 supportate"].
...
|===
Consulta link:../s3/index.html#supported-versions["Versioni e aggiornamenti supportati dall'API REST S3"] per maggiori informazioni.
