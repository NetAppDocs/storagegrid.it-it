...
* Se prevedi di utilizzare richieste S3 in stile virtual-hosted, hai link:../admin/configuring-s3-api-endpoint-domain-names.html["Nomi di dominio degli endpoint S3 configurati"]. ONTAP utilizza URL in stile percorso per impostazione predefinita, ma è consigliato usare richieste in stile virtual-hosted.
