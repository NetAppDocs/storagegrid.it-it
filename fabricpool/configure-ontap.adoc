...
---
= Configura ONTAP System Manager per aggiungere StorageGRID come livello cloud FabricPool
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Stile URL
 a| 
Se link:../admin/configuring-s3-api-endpoint-domain-names.html["Nomi di dominio degli endpoint S3 configurati"] selezioni *Virtual-Hosted-Style URL*.

*URL in formato percorso* è l'impostazione predefinita per ONTAP, ma è consigliato utilizzare richieste in formato virtual-hosted-style per StorageGRID. Devi usare *URL in formato percorso* se fornisci un indirizzo IP invece di un domain name per il campo *Server name (FQDN)*.
