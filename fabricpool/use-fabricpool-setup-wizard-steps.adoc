...
---
= Accedi e completa il setup wizard di StorageGRID FabricPool
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Definisci l'accesso root per l'account tenant, in base all'utilizzo o meno da parte del sistema StorageGRIDlink:../admin/using-identity-federation.html["federazione delle identità"] ,link:../admin/how-sso-works.html["SSO (Single Sign-on)"] , o entrambi.
+
[cols="1a,3a"]
|===
...
.Fasi
. Facoltativamente, seleziona l'icona di copia (image:icon_tenant_copy_url.png["icona di copia"]) per copiare negli appunti sia l'ID della chiave di accesso che la chiave di accesso segreta.
+
