...
.Al termine
Se hai aggiunto un nuovo sito o un nuovo livello di storage a un pool di storage utilizzato nella policy ILM attiva, attiva una nuova policy ILM per forzare StorageGRID a utilizzare il nuovo sito o il nuovo livello di storage. Ad esempio, clona la tua policy ILM esistente e poi attiva il clone. Fai riferimento a link:working-with-ilm-rules-and-ilm-policies.html["Utilizzare le regole ILM e i criteri ILM"].
