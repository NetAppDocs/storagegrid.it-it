...
NOTE: Quando installi StorageGRID 12.0 o versioni successive su una nuova griglia, i pool di storage vengono creati automaticamente per ogni sito. Tuttavia, se inizialmente hai installato StorageGRID 11.6 o versioni precedenti, i pool di storage non vengono creati automaticamente per ogni sito.
...
+
Per rimuovere una voce, seleziona l'icona di eliminazione image:icon_x_to_remove.png["icona elimina"].
