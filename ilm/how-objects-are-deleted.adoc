...
Se un client elimina una versione di un oggetto con un ID versione da unlink:../tenant/what-is-branch-bucket.html["secchio di rami"] StorageGRID nasconde l'esistenza della versione dell'oggetto ma non crea un marcatore di eliminazione nel bucket del ramo.  Se un client elimina un oggetto senza un ID versione da un bucket di diramazione, StorageGRID crea un marcatore di eliminazione nel bucket di diramazione in base al comportamento S3 standard.
