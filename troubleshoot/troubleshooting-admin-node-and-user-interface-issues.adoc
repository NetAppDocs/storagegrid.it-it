...
---
= Risolvi i problemi relativi al nodo di amministrazione e all'interfaccia utente in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Se il single sign-on (SSO) è abilitato per il tuo sistema StorageGRID, consulta la procedura per configurare l'SSO usando link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] o link:../admin/learn-about-setting-up-sso-saml.html["SAML"].
+
...
... Seleziona *Verifica connessione* per convalidare le impostazioni di connessione per il LDAP server.
