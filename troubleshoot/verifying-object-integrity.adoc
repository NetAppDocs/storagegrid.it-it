...
---
= Verificare l'integrità dei dati degli oggetti in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
Questa procedura è simile a quella perlink:../troubleshoot/investigating-potentially-lost-objects.html["investigare oggetti potenzialmente persi"] , sebbene per le copie degli oggetti si cerchi `LLST` invece di `OLST` .
