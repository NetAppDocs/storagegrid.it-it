...
summary: 'Potrebbe essere necessario decommissionare un nodo che non è attualmente connesso alla rete (un nodo il cui stato di salute è sconosciuto o amministrativamente inattivo).' 
---
...
Puoi identificare i nodi disconnessi cercando l'icona blu "Sconosciuto" image:icon_alarm_blue_unknown.png["Icona sconosciuta"] o l'icona grigia "Amministrativamente non disponibile" image:icon_alarm_gray_administratively_down.png["Icona amministrativamente giù"] nella colonna *Integrità*.
...
. Seleziona *Avvia decommissionare*.
+
...
. Esamina l'elenco dei nodi e seleziona *OK*.
+
...
. Non appena il nuovo pacchetto di ripristino sarà disponibile, seleziona il collegamento oppure vai su *Manutenzione* > *Sistema* > *Pacchetto di ripristino* per accedere alla pagina del pacchetto di ripristino. Quindi, scarica il  `.zip` file.
+
...
. Se stai decommissionando un Storage Node, monitora lo stato dei processi di riparazione dei *dati replicati* e dei *dati con codifica di cancellazione (EC)* che vengono avviati automaticamente durante il processo di decommissionamento.
+
[role="tabbed-block"]
====
.Dati replicati
...
--
Per monitorare la riparazione dei dati codificati per la cancellazione e riprovare le richieste che potrebbero non essere riuscite, seleziona *Supporto* > *Strumenti* > *Metriche* per visualizzare il tempo stimato di completamento e la % di completamento per il job corrente. Poi seleziona *EC Overview* nella sezione Grafana. Guarda le dashboard *Grid EC Job Estimated Time to Completion* e *Grid EC Job Percentage Completed*.
