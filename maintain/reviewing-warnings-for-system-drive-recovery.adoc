...
---
= Avvisi per il ripristino dell'unità di sistema del nodo di storage StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
CAUTION: Se questo Storage Node è in modalità di manutenzione di sola lettura per consentire il recupero degli oggetti da parte di un altro Storage Node con volumi di storage guasti, ripristina i volumi sullo Storage Node con volumi di storage guasti prima di ripristinare questo Storage Node guasto. Vedi le istruzioni per link:reviewing-warnings-about-storage-volume-recovery.html["ripristino in seguito a un errore del volume di storage in cui il disco di sistema è intatto"].
