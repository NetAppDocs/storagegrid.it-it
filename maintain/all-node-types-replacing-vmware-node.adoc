...
---
= Sostituisci un nodo VMware in StorageGRID con un nodo di ripristino
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Distribuisci una nuova macchina virtuale come nodo sostitutivo e collegala a una o più reti StorageGRID .  Per le istruzioni vederelink:../swnodes/deploying-storagegrid-node-as-virtual-machine.html["Implementazione di un nodo StorageGRID come macchina virtuale"] .
+
...
Nodi di amministrazione
 a| 
link:recovering-from-admin-node-failures.html["Ripristino del nodo Admin primario o non primario in StorageGRID"]
