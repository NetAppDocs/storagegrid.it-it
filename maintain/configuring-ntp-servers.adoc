...
---
= Gestisci i server NTP in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Quando specifichi l'origine NTP esterna per un'installazione di StorageGRID a livello di produzione, non usare il servizio Ora di Windows (W32Time) su una versione di Windows precedente a Windows Server 2016. Il servizio Ora sulle versioni precedenti di Windows non è sufficientemente preciso e non è supportato da Microsoft per l'uso in ambienti ad alta precisione, incluso StorageGRID. Per dettagli, vedi  https://learn.microsoft.com/en-us/troubleshoot/windows-server/active-directory/support-boundary-high-accuracy-time["Limite di supporto per un tempo ad alta precisione"^].
