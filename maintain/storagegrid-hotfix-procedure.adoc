...
Dopo che un hotfix è stato applicato alla tua grid, il nodo Admin primario o un nodo Admin non primario installa automaticamente la stessa versione dell'hotfix su qualsiasi nodo ripristinato tramite operazioni di recovery o aggiunto in un'espansione. Puoi recuperare qualsiasi nodo (incluso il nodo Admin primario) quando hai effettuato l'accesso a un nodo Admin non primario. Se recuperi un nodo da un nodo Admin non primario, quel nodo Admin applica l'hotfix al nodo recuperato.

Se devi ripristinare il nodo Admin primario dal suo programma di installazione di StorageGRID, devi installare manualmente la versione corretta di StorageGRID e poi applicare l'hotfix. La versione finale di StorageGRID del nodo Admin primario deve corrispondere alla versione degli altri nodi nella grid.

L'esempio seguente illustra come applicare una hotfix durante il ripristino del nodo Admin primario mentre sei connesso al nodo Admin non primario:
