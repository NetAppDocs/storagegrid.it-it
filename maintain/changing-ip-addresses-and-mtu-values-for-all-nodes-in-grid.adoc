...
---
= Modifica gli indirizzi IP per tutti i nodi in una griglia StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Se il single sign-on (SSO) è abilitato per il tuo sistema StorageGRID e qualsiasi relying party trust è stato configurato utilizzando gli indirizzi IP del nodo Admin (anziché i fully qualified domain name, come raccomandato), preparati ad aggiornare o riconfigurare questi relying party trust in Active Directory Federation Services (AD FS) immediatamente dopo aver cambiato gli indirizzi IP. Consulta la guida per configurare l'SSO usando link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] o link:../admin/learn-about-setting-up-sso-saml.html["SAML"].
