...
---
= Ripristina dai guasti dei nodi di archiviazione StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Contattare il supporto tecnico.
 a| 
Se si verifica un guasto a più di un nodo di archiviazione, il supporto tecnico deve intervenire per il ripristino al fine di prevenire incoerenze nel database Cassandra che potrebbero causare la perdita di dati. Potrebbe essere necessaria una procedura di ripristino del sito.

Il supporto tecnico può stabilire quando è sicuro avviare il ripristino di un secondo Storage Node.

*Nota*: Se più di un nodo di archiviazione che contiene il servizio ADC si guasta in un sito, tutte le richieste di servizio della piattaforma in sospeso per quel sito andranno perse.
...
Uno o più volumi di storage sono guasti, ma il disco di sistema è intatto
 a| 
link:reviewing-warnings-about-storage-volume-recovery.html["Ripristino in seguito a un errore del volume di storage in cui il disco di sistema è intatto"]
 a| 
