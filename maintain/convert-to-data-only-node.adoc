...
---
= Convertire un nodo di storage StorageGRID in un nodo di soli dati
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Se il nodo di archiviazione contiene il servizio ADC, più di due nodi di archiviazione ADC sono online nella griglia e nodi di archiviazione non ADC sono disponibili nello stesso sito per ospitare il servizio ADC.
...
Usa questa procedura per convertire uno o più nodi di archiviazione combinati in nodi solo dati. Se un nodo di archiviazione contiene il servizio ADC, il servizio viene automaticamente spostato su un altro nodo di archiviazione nello stesso sito durante la conversione.
...
NOTE: StorageGRID sposta il servizio ADC sui nodi di storage più recenti che soddisfano i requisiti per l'hosting del servizio.
