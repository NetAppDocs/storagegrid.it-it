...
* Conosci l'indirizzo IP di uno dei seguenti Admin Node:
+
** Il nodo Admin principale per la grid StorageGRID
** Un nodo Admin non primario, se disponibile
...
* Specifichi l'indirizzo IP del nodo Admin primario. Se il nodo Admin primario non è disponibile, specifichi l'indirizzo IP del nodo Admin non primario, se disponibile. Confermi anche il nome host (nome di sistema) del nodo.
...
. Nel campo *Nome nodo*, inserisci lo stesso hostname (nome di sistema) che hai usato per il nodo che stai recuperando e seleziona *Salva*.
. Nella sezione Installazione, verifica che lo stato corrente sia "Pronto per avviare l'installazione di `_node name_` nella griglia con il nodo amministratore primario `_admin_ip_`" e che il pulsante *Avvia installazione* sia abilitato.
+
...
NOTE: Se devi accedere manualmente alla pagina di installazione del Monitor, seleziona *Installazione del Monitor* dalla barra dei menu. Vedi  https://docs.netapp.com/us-en/storagegrid-appliances/installconfig/monitoring-appliance-installation.html["Installazione dell'appliance di monitoraggio"^].
