...
---
= Accedi alla pagina Decommissiona nodi in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
In questo esempio, un segno di spunta verde nella colonna Dismissione possibile indica che puoi decommissionare il nodo API Gateway e il nodo di Storage solo dati. Gli altri nodi non possono essere decommissionati per vari motivi. Ad esempio, il nodo Admin primario non può essere decommissionato perché la decommissione di questo tipo di nodo non è supportata. Il nodo di Storage solo metadati non può essere decommissionato perché il sito non soddisferebbe più i link:../maintain/understanding-adc-service-quorum.html["Quorum ADC"].

+
image:decommission_nodes_page.png["Pagina Decommissionate Nodes"]
...
** image:icon_alarm_gray_administratively_down.png["icona del punto interrogativo grigio"] (grigio): Amministrativamente chiuso
** image:icon_alarm_blue_unknown.png["icona del punto interrogativo blu"] (blu): Sconosciuto
...
Modificare il gruppo ha per rimuovere l'interfaccia del nodo o rimuovere l'intero gruppo ha. Vedere link:../admin/configure-high-availability-group.html["Configurare i gruppi ad alta disponibilità"].
 ...
 a| 
No, il sito _x_ necessita di almeno 3 Storage Node con servizi ADC o di nodi non-ADC disponibili su cui spostare i servizi ADC. Il decommissionare del nodo ADC non è disponibile se uno qualsiasi dei nodi è offline.
 a| 
*Solo nodi di archiviazione* (nodi di archiviazione combinati o solo metadati). Non puoi decommissionare un nodo di archiviazione se nel sito rimangono nodi insufficienti a supportare i requisiti di quorum ADC e il servizio ADC non può essere spostato automaticamente. Lo spostamento automatico richiede che siano online più di due nodi di archiviazione ADC nella grid e che siano disponibili nodi di archiviazione non ADC nello stesso sito.
 a| 
Assicurati che siano soddisfatte le condizioni per lo spostamento automatico del servizio ADC (consulta link:../maintain/understanding-adc-service-quorum.html["Quorum ADC"]), oppure link:../maintain/move-adc-service.html["spostare manualmente il servizio ADC"] verso un altro Storage Node nello stesso sito. Puoi anche eseguire un'espansione aggiungendo un nuovo Storage Node al sito e specificando che deve avere un servizio ADC. Consulta link:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.html["Riepilogo del flusso di lavoro per l'aggiunta di nodi griglia a StorageGRID"].
