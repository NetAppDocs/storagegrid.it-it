...
summary: 'Dalla fase 5 (Risolvi conflitti di nodi) della procedura guidata Smantella sito, è possibile determinare se i nodi nel sistema StorageGRID sono disconnessi o se i nodi nel sito selezionato appartengono a un gruppo ad alta disponibilità (ha). Una volta risolti i conflitti di nodo, avviare la procedura di decommissionamento da questa pagina.' 
---
= Risolvere i conflitti tra i nodi e avviare la decommission in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Tutti i nodi nel tuo sistema StorageGRID devono essere connessi (image:icon_alert_green_checkmark.png["Icona Avviso segno di spunta verde"]).
+
...
.Fasi
. Esamina la sezione relativa ai nodi disconnessi del passaggio 5 (Risolvi i conflitti tra nodi) per determinare se uno qualsiasi dei nodi nel tuo sistema StorageGRID ha uno stato di connessione Unknown (image:icon_alarm_blue_unknown.png["Icona allarme blu sconosciuto"]) o Administratively Down (image:icon_alarm_gray_administratively_down.png["Icona Alarm Grey amministrativamente in basso"]).
+
image:decommission_site_step_5_disconnected_nodes.png["Decommissionare il sito fase 5 risolvere i conflitti dei nodi"]
...
+
image:decommission_site_step_5_ha_groups.png["Decommissionare i gruppi ha fase 5 del sito"]
...
+
Se tutti i nodi sono connessi e nessun nodo del sito selezionato viene utilizzato in un gruppo HA, il campo *Provisioning passphrase* è abilitato.
...
+
image:decommission_site_step_5_provision_passphrase.png["Decommissionare il sito fase 5 eseguire il provisioning della passphrase"]
...
+
image:decommission_site_step_5_warning.png["Decommissionare il sito fase 5 attenzione"]
...
+
image:decommission_site_step_5_grid_configuration.png["Decommissionare il sito fase 5 Configurazione griglia"]
