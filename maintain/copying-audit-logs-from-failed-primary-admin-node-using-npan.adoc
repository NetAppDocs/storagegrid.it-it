...
---
= Copia i log di audit di StorageGRID da un nodo Admin primario guasto
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.. Inserisci il seguente comando per passare all'account utente root: `su -`
.. Immettere la password elencata nel `Passwords.txt` file.
+
...
+
Quando richiesto, inserisci la password per l'account utente admin.
...
. Disconnettiti come utente root: `exit`.
