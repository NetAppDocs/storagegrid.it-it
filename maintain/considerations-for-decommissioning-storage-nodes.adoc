...
---
= Considerazioni sulla decommission dei nodi di archiviazione in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Il sistema deve, in ogni momento, includere un numero sufficiente di Storage Node per soddisfare i requisiti operativi, inclusi il link:understanding-adc-service-quorum.html["Quorum ADC"] e l'attivo link:reviewing-ilm-policy-and-storage-configuration.html["Policy ILM"]. Se una decommission dovesse portare il conteggio degli ADC di un sito al di sotto del quorum, StorageGRID sposta automaticamente il servizio ADC sui Storage Node non-ADC disponibili nello stesso sito, mantenendo il quorum ADC. Se il servizio ADC non può essere spostato automaticamente, potresti doverlo link:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.html["aggiungi un nuovo nodo di archiviazione in un'operazione di espansione"] manualmente prima di poter decommissionare uno Storage Node esistente.
+
