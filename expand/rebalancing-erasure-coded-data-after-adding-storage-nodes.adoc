...
summary: Dopo che hai aggiunto i nodi di archiviazione, puoi usare la procedura di ribilanciamento della codifica di cancellazione per ridistribuire i frammenti codificati con codifica di cancellazione tra i nodi di archiviazione esistenti e quelli nuovi. 
---
= Ribilancia i dati codificati in modo da essere cancellati dopo aver aggiunto nodi al tuo sistema StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
** I dati in tutti i nodi rientrano in una deviazione del 5% rispetto alla partizione di spazio utilizzabile target.
...
+
Il processo di ribilanciamento EC si considera completato e si interromperà quando lo spazio utilizzabile in tutti i nodi rientrerà in una deviazione del 5% rispetto alla partizione di spazio utilizzabile target.
...
+
Lo spazio utilizzabile dovrebbe ora essere più bilanciato tra i Storage Node nel sito.
