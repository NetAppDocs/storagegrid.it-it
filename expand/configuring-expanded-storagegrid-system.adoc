...
---
= Configurare i nodi e i siti appena aggiunti in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
.. Se il single sign-on (SSO) è abilitato per il tuo sistema StorageGRID, crea una relying party trust per il nuovo Admin Node. Non puoi accedere al nodo finché non crei questa relying party trust. Consulta come configurare l'SSO usando link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] o link:../admin/learn-about-setting-up-sso-saml.html["SAML"].
