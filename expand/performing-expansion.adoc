...
---
= Eseguire un'espansione StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Fasi
. Seleziona *Manutenzione* > *Attività* > *Espansione griglia*.
+
Viene visualizzata la pagina di espansione della griglia. La sezione Nodi in sospeso elenca i nodi pronti per essere aggiunti.
...
+
image:grid_node_details.png["Schermata dei dettagli del nodo di griglia"]
...
. Dopo aver approvato tutti i nodi della griglia, inserisci la *passphrase di provisioning* e seleziona *Espandi*.
+
