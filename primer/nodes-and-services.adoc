...
Nodi di amministrazione:: Fornisci servizi di gestione come la configurazione del sistema, il monitoraggio e la registrazione. Quando accedi al Grid Manager, ti connetti a un Admin Node. Ogni grid deve avere un Admin Node primario e può avere Admin Node non primari aggiuntivi per la ridondanza. Puoi connetterti a qualsiasi Admin Node e ogni Admin Node mostra una vista simile del sistema StorageGRID. Tuttavia, devi eseguire la maggior parte delle procedure di manutenzione utilizzando l'Admin Node primario.
+
--
...
+
--
Durante l'installazione iniziale di un nuovo nodo di archiviazione, puoi specificare che venga utilizzato solo per link:../primer/what-storage-node-is.html#types-of-storage-nodes["memorizzare sia metadati che dati"].
...
gdu-server
 a| 
Il task runner di backend per le operazioni di manutenzione e provisioning della griglia. Accetta richieste da  `mgmt-api` e da altri script di utilità, le convalida e inoltra ogni azione al gestore di attività GDU appropriato, come provisioning, espansione, decommission, e aggiornamenti.
 a| 
Nodi di amministrazione
...
Alta disponibilità
 a| 
Gestisce gli indirizzi IP virtuali ad alta disponibilità sui nodi configurati nella pagina Gruppi ad alta disponibilità. Questo servizio è anche noto come servizio _keepalived_.
 a| 
...
mgmt-api
 a| 
L'API di backend di Grid Manager. Espone endpoint REST autenticati per l'amministrazione della grid (configurazione, monitoraggio e workflow di manutenzione), applica convalide e autorizzazioni e orchestra operazioni di lunga durata. L'API principale del control-plane utilizzata dall'interfaccia di Grid Manager e dai client di automazione.
 a| 
Nodi di amministrazione
