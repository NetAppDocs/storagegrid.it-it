...
Puoi abilitare o disabilitare l'accesso IPMI remoto per tutti gli appliance che contengono un BMC. L'interfaccia IPMI remota consente l'accesso hardware di basso livello ai tuoi appliance StorageGRID a chiunque abbia un account BMC e una password. Se non ti serve l'accesso IPMI remoto al BMC, disattiva questa opzione.

* Per controllare l'accesso IPMI remoto al BMC in Grid Manager, vai su *Configuration* > *Security* > *Security settings* > *Appliances* > *Remote IPMI access*:
+
...
== Cambia la password BMC dell'appliance

Puoi cambiare la password BMC di un'appliance per limitare l'accesso e migliorare la sicurezza del sistema.

. In Grid Manager vai su *Nodi* > *_nodo griglia_* > *Impostazioni BMC*.
. Nella sezione Cambia password, inserisci una nuova password BMC di lunghezza compresa tra 8 e 16 caratteri nel campo *Nuova password* e poi inseriscila nuovamente nel campo *Conferma nuova password*.
. Selezionare *Salva*.


Puoi anche cambiare la password BMC usando l' `PUT /bmc-configuration/root-password`API.



[[control-remote-access-through-usb-portscontrol-external-usb-port-access-on-appliances]]
== [[control-remote-access-through-usb-ports]]Controlla l'accesso alle porte USB esterne sugli appliance

Puoi abilitare le porte USB sugli appliance per consentire al supporto tecnico l'accesso per la diagnostica e la risoluzione dei problemi. Se non ti serve l'accesso alle porte USB, disabilita le porte per sicurezza. Le porte USB sono abilitate per impostazione predefinita dopo l'installazione iniziale o un upgrade.

* In Grid Manager, vai su *Configurazione* > *Sicurezza* > *Impostazioni di sicurezza* > *Apparecchiature* > *Accesso alla porta USB* per abilitare o disabilitare le porte USB:
+
** Seleziona la casella di controllo *Abilita porte USB* e salva per abilitare le porte.
+
Se la casella di controllo USB era stata precedentemente deselezionata e salvata, seleziona la casella di controllo *Abilita porte USB* e salva per abilitare di nuovo le porte USB.

** Deseleziona la casella *Abilita porte USB* e salva per disabilitare le porte.
...
* Non consentire ai client non attendibili di connettersi direttamente ai Storage Node. Usa un endpoint di bilanciamento del carico fornito da un Gateway Node o da un bilanciatore del carico di terze parti.
