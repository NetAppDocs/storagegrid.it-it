...
. Nella pagina dei dettagli dell'endpoint, seleziona *Connessione*. Questa scheda mostra solo l'errore più recente per un endpoint e indica quanto tempo fa si è verificato. Gli errori che includono l'icona X rossa image:icon_alert_red_critical.png["Icona X rossa"] si sono verificati negli ultimi 7 giorni.
