...
summary: 'Quando gli amministratori della griglia creano le regole di gestione del ciclo di vita delle informazioni (ILM) per un sistema StorageGRID, possono facoltativamente specificare che l"ora dell"ultimo accesso di un oggetto venga utilizzata per determinare se spostare tale oggetto in una posizione di storage diversa. Se usi un tenant S3, puoi sfruttare queste regole abilitando gli aggiornamenti dell"ora dell"ultimo accesso per gli oggetti in un bucket S3.' 
---
