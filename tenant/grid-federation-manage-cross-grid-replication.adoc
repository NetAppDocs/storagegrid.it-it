...
summary: 'Se al tuo tenant account è stata assegnata l"autorizzazione Use grid federation connection al momento della creazione, puoi utilizzare la replica tra griglie per replicare automaticamente gli oggetti tra i bucket sulla tenant source grid e i bucket sulla tenant destination grid. La replica tra griglie può avvenire in una o entrambe le direzioni.' 
---
...
+
image:grid_federation_cross_grid_replication_client_deletes.png["immagine che mostra l'eliminazione del client replicato su entrambe le griglie"]
