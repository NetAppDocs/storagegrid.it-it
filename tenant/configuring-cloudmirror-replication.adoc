...
* La replica è supportata solo per i bucket di origine o di destinazione con S3 Object Lock abilitato per i bucket Amazon S3 o StorageGRID S3. Le intestazioni di Object Lock non vengono replicate nell'oggetto di destinazione.
...
* Se abiliti S3 Object Lock sul bucket di destinazione dopo aver configurato la replica CloudMirror, potresti dover link:../tenant/editing-platform-services-endpoint.html["salva nuovamente l'endpoint"] per consentire la ripresa della replica.
