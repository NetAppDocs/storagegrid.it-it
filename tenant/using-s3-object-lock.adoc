...
Quando l'impostazione globale S3 Object Lock è abilitata per un sistema StorageGRID, un account tenant S3 può creare bucket con o senza S3 Object Lock abilitato. Quando l'impostazione allow S3 Object Lock for existing buckets è abilitata, un account tenant S3 può abilitare S3 Object Lock per un bucket esistente. Se un bucket ha S3 Object Lock abilitato, il versioning del bucket è richiesto e viene abilitato automaticamente. Dopo che un account tenant S3 ha abilitato S3 Object Lock per un bucket nuovo o esistente, non può essere disabilitato.
...
+
Per i dettagli sulle impostazioni dell'oggetto, fai riferimento a link:../s3/use-s3-api-for-s3-object-lock.html["Utilizzare l'API REST S3 per configurare il blocco oggetti S3"].
...
Fai riferimento a link:../tenant/creating-s3-bucket.html["Creare un bucket S3"] e link:../tenant/update-default-retention-settings.html["Aggiorna la conservazione predefinita del blocco oggetti S3"].
...
Scopri di piùlink:../ilm/requirements-for-s3-object-lock.html#s3-object-lock-existing-buckets["Requisiti di S3 Object Lock"]
