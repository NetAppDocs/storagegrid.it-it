...
+
image:grid_federation_post_current_user_replicate.png["POST endpoint per la replica della propria chiave di accesso"]
...
+
image:grid_federation_clone_access_key.png["API di tenant Manager che immette i valori per clonare la chiave di accesso"]
...
+
image:grid_federation_post_other_user.png["ENDPOINT POST per la replica della chiave di accesso di un altro utente"]
