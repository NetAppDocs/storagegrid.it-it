...
summary: 'Se il tuo StorageGRID tenant account dispone dell"autorizzazione "Usa connessione di federazione grid", puoi utilizzare l"API di gestione tenant per clonare manualmente le chiavi di accesso S3.' 
---
...
+
image:grid-federation-post-current-user-replicate.png["POST endpoint per la replica della propria chiave di accesso"]
...
+
image:grid-federation-clone-access-key.png["API di tenant Manager che immette i valori per clonare la chiave di accesso"]
...
+
image:grid-federation-post-other-user.png["ENDPOINT POST per la replica della chiave di accesso di un altro utente"]
