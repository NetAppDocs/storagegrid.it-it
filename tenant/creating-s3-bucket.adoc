...
* Se più tenant avranno ciascuno fino a 5.000 bucket, ogni Storage Node nella griglia avrà un minimo di 64 GB di RAM.
+

NOTE: Il sistema StorageGRID supporta un massimo di 20.000 bucket per ogni account tenant S3. Ogni grid può avere un massimo di 100.000 bucket, inclusi link:../tenant/what-is-branch-bucket.html["secchi di rami"]. C'è anche un limite a link:../tenant/manage-branch-buckets.html["profondità del bucket del branch"].
...
Per ulteriori informazioni, consulta il  https://docs.aws.amazon.com/AmazonS3/latest/userguide/bucketnamingrules.html["Documentazione di Amazon Web Services (AWS) sulle regole di denominazione del bucket"^].
