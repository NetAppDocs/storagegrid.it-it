...
summary: 'Puoi eseguire l"upgrade a StorageGRID 12.1 e applicare l"ultimo hotfix per quella release contemporaneamente. La pagina di upgrade di StorageGRID fornisce il percorso di upgrade consigliato e i link diretti alle pagine di download corrette.' 
---
= Eseguire un aggiornamento StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Puoi eseguire l'upgrade a StorageGRID 12.1 e applicare l'ultimo hotfix per quella release contemporaneamente. La pagina di upgrade di StorageGRID fornisce il percorso di upgrade consigliato e i link diretti alle pagine di download corrette.
...
Il percorso di upgrade nella pagina Upgrade di StorageGRID indica quali versioni principali (per esempio, 12.1.0) e hotfix (per esempio, 12.1.0.1) devi installare per arrivare all'ultima release di StorageGRID. Dovresti installare le versioni e gli hotfix raccomandati nell'ordine mostrato.
...
. Seleziona *Sfoglia* per caricare il file di upgrade della versione su StorageGRID: `NetApp_StorageGRID_12.1.0_Software_uniqueID.upgrade`
+
...
. Esamina i dettagli di ogni precontrollo segnalato e risolvi eventuali errori. Consulta la  https://kb.netapp.com/hybrid/StorageGRID/Maintenance/StorageGRID_12.1_software_upgrade_resolution_guide["Guida alla risoluzione degli aggiornamenti del software StorageGRID"^] per la release StorageGRID 12.1.
+
...
NOTE: Per un upgrade a StorageGRID 12.1, se vuoi approvare singoli nodi invece di tutti i nodi, la best practice è aggiornare un intero sito prima di passare al sito successivo.
...
.Fasi
. Non appena l'attività *Abilita funzionalità* sarà completata (fino a un'ora), puoi iniziare a usare la link:whats-new.html["nuove funzionalità"] nella versione aggiornata di StorageGRID.
+
