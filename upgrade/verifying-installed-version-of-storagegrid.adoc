...
---
= Verificare la versione installata di StorageGRID prima dell'aggiornamento
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Prima di aggiornare a StorageGRID 12.1, sulla tua grid deve essere installato StorageGRID 12.0 o una versione successiva. Se stai utilizzando una versione precedente di StorageGRID, devi installare tutti i file di upgrade precedenti insieme ai relativi hotfix più recenti (fortemente consigliato) finché la versione attuale della tua grid non sarà almeno StorageGRID 12.0.0.
...
L'esempio seguente mostra i passaggi per eseguire l'upgrade da StorageGRID versione 11.9 alla versione 12.0 in preparazione per un upgrade a StorageGRID 12.1.
