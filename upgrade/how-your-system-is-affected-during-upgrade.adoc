...
---
= In che modo gli aggiornamenti di StorageGRID influiscono sul tuo sistema
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Durante l'upgrade a StorageGRID 12.1:

* Tutti i nodi dell'appliance StorageGRID vengono aggiornati automaticamente al firmware StorageGRID Appliance Installer versione 12.1.
...
* Gli appliance SGF6112, SG6160, SG110 e SG1100 vengono aggiornati automaticamente alla versione firmware BIOS 3A14.QD e alla versione firmware BMC 3.21.07.
...
Ad esempio, potresti visualizzare l'avviso *Impossibile comunicare con il nodo* quando i servizi vengono arrestati, oppure l'avviso *Errore di comunicazione Cassandra* quando alcuni nodi sono stati aggiornati a StorageGRID 12.0 o versioni successive, ma altri nodi eseguono ancora una versione precedente di StorageGRID. In generale, questi avvisi si risolvono quando l'upgrade è completato.

L'avviso *Impossibile posizionare ILM* potrebbe essere attivato quando i nodi di archiviazione vengono arrestati durante l'upgrade a StorageGRID 12.0 o versioni successive. Questo avviso potrebbe persistere per 1 giorno dopo il completamento dell'upgrade.
...
NOTE: Le restrizioni relative alle modifiche di configurazione sono specifiche per ogni versione. Se stai eseguendo l'upgrade a un'altra release di StorageGRID, consulta l'elenco delle modifiche non consentite nelle istruzioni di upgrade per quella release.
...
* Non dovresti gestire gruppi HA, interfacce VLAN o endpoint del bilanciatore di carico mentre stai eseguendo l'upgrade.
* Non eliminare alcun gruppo HA finché l'upgrade non è completato. Gli indirizzi IP virtuali in altri gruppi HA potrebbero diventare inaccessibili.
