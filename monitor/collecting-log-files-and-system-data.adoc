...
Se il nodo di amministrazione utilizzato per avviare la raccolta dei log va offline durante la raccolta, StorageGRID non conserva i risultati. Devi riavviare il nodo e riavviare la raccolta dei log al termine del riavvio.
...
NOTE: Puoi lasciare vuoto il campo della passphrase di provisioning per un tentativo standard oppure inserire la passphrase per riprovare tramite SSH.
