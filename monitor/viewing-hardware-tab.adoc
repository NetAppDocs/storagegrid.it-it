...
image:nodes_page_hardware_tab_graphs.png["Scheda hardware della pagina nodi"]
...
image:nodes_page_memory_usage_details.png["Pagina dei nodi > Hardware > Dettagli sull'utilizzo della memoria"]
...
+
image:nodes_page_overview_tab_extended.png["Panoramica pagina nodi estesa"]
...
+
image:nodes_page_hardware_tab_graphs.png["Grafici hardware"]
...
Lo stato della temperatura del controller di calcolo.
 ...
 a| 
Alimentatore del controller di calcolo A
 a| 
Stato dell'alimentatore A per il controller di calcolo.
...
Alimentatore del controller di calcolo B
 a| 
Lo stato dell'alimentatore B per il controller di calcolo.
...
+
image:nodes_page_network_traffic_graph.png["Pagina nodi grafico traffico di rete"]

+
.. Controlla la sezione Interfacce di rete.
+
image:nodes_page_network_interfaces.png["Interfacce di rete della pagina Nodes (nodi)"]
...
.. Controlla la sezione Comunicazione di rete.
+
...
+
image:nodes_page_storage_used_object_data.png["Storage utilizzato - dati oggetto"]

+
image:storage_used_object_metadata.png["Storage utilizzato - metadati oggetto"]
...
+
image:nodes_page_overview_tab_services_appliance.png["Scheda Panoramica della pagina nodi per l'appliance di servizi"]
...
. Seleziona la scheda *Hardware* per visualizzare ulteriori informazioni sull'appliance.
+
...
+
image:nodes_page_hardware_tab_graphs_services_appliance.png["Pagina nodi grafici della scheda hardware per l'appliance di servizi"]

.. Scorri verso il basso per visualizzare la tabella dei componenti dell'appliance. La tabella contiene informazioni quali il nome del modello, il numero di serie, la versione del firmware del controller e lo stato di ciascun componente.
+
[cols="1a,2a"]
|===
| Nella tabella Appliance | Descrizione 
...
. Seleziona la scheda *Rete* per visualizzare le informazioni per ciascuna rete.
+
Il grafico del traffico di rete fornisce un riepilogo del traffico di rete complessivo.
...
.. Controlla la sezione Interfacce di rete.
+
Usa la seguente tabella con i valori nella colonna *Velocità* della tabella Interfacce di rete per determinare se le quattro porte di rete 40/100-GbE sull'appliance sono state configurate per utilizzare la modalità attivo/backup o la modalità LACP.
...
. Seleziona la scheda *Archiviazione* per visualizzare le informazioni sui dispositivi disco e sui volumi sull'appliance di servizio.
