...
| acct-tunnel.log  a| 
Registro per l'helper del tunnel account che funziona con il bilanciatore di carico.
...
Registro interno del traffico proxy HTTP.
...
| manage-audit.log  a| 
`mgmt-api`accedi ai registri di controllo.
...
* stream-access.log: sessioni proxy TCP/TLS stream (livello 4) per il blocco del flusso.
+
*Nota*: Questo non è un registro delle richieste HTTP.
...
Registro degli accessi per il traffico di federazione della griglia instradato come S3 (URL di spazio dei nomi globale/federazione).
...
| system-bucket.log  a| 
`update-system-bucket-configuration.log`: Log del bucket di sistema.
...
`update-cors-configurations.log`: Traccia i log relativi alle modifiche di TLS/cifratura e CORS.



| update-leakd-configurations.log  a| 
Registro di configurazione del rilevamento degli orfani.
