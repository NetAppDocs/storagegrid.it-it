...
+
image:nodes_menu.png["Menu nodi"]
...
+
image:traffic_classification_policies_main_screen_w_examples.png["Esempio di policy sul traffico per il grafico"]

.. Per visualizzare i grafici che mostrano le metriche di rete associate a una policy, seleziona il pulsante di opzione a sinistra della policy e poi seleziona *Metriche*.
