...
+
image:tenant_accounts_page.png["Pagina account tenant"]

. Facoltativamente, effettua il Sign in a un tenant account selezionando il link di Sign in image:icon_tenant_sign_in.png["Icona di accesso tenant"] nella colonna *Sign in/Copy URL*.
. Facoltativamente, puoi copiare l'URL della pagina di Sign in di un tenant selezionando il link Copia URL image:icon_tenant_copy_url.png["Icona URL copia tenant"] nella colonna *Sign in/Copia URL*.
...
+
image:tenant_accounts_example_csv.png["Esempio di account tenant .csv"]
...
+
image:tenant_usage_modal.png["Modale di utilizzo del tenant"]
...
+
image:tenant_bucket_space_consumption_GM.png["Grafico a barre del consumo del bucket tenant"]
...
+
image:tenant_bucket_details_csv.png["Esempio CSV dettagli bucket tenant"]
