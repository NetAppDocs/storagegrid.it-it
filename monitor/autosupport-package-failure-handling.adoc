...
summary: 'Esamina l"elenco delle azioni che StorageGRID intraprende se un tentativo di invio di un pacchetto AutoSupport fallisce.' 
---
= Come StorageGRID gestisce gli errori dei pacchetti AutoSupport
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Esamina l'elenco delle azioni che StorageGRID intraprende se un tentativo di invio di un pacchetto AutoSupport fallisce.

StorageGRID esegue azioni diverse a seconda del tipo di AutoSupport package. Puoi controllare lo stato dei pacchetti AutoSupport selezionando *Support* > *Tools* > *AutoSupport* > *Results*.
...
NOTE: Se hai configurato un proxy server per inoltrare i pacchetti AutoSupport a NetApp, link:../admin/configuring-admin-proxy-settings.html["verificare che le impostazioni di configurazione del server proxy siano corrette"].
...
Se si verifica un errore e il protocollo selezionato è SMTP, sulla pagina AutoSupport potrebbe apparire il seguente messaggio di errore: `AutoSupport packages cannot be sent using SMTP protocol due to incorrect settings on the E-mail Server page.`
...
Se si verifica un errore e il protocollo selezionato è SMTP, verifica che il server di posta elettronica del sistema StorageGRID sia configurato correttamente e che il tuo server di posta elettronica sia in esecuzione. Potrebbe apparire il seguente messaggio di errore nella pagina AutoSupport: i pacchetti AutoSupport non possono essere inviati tramite il protocollo SMTP a causa di impostazioni errate nella pagina E-mail Server.
