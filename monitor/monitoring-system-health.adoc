...
image:health_status_card.png["Scheda di stato dello stato di salute - dashboard"]
...
| Icona | Descrizione | Azione richiesta 
 ...
 a| 
image:icon_alarm_blue_unknown.png["icona del punto interrogativo blu"]
 a| 
...
*Nota*: Un nodo potrebbe apparire come sconosciuto durante le operazioni di shutdown gestite. In questi casi, è possibile ignorare lo stato Unknown (Sconosciuto).
 ...
 a| 
image:icon_alarm_gray_administratively_down.png["icona del punto interrogativo grigio"]
 a| 
...
Se sono attivi uno o più avvisi, <<Visualizzare gli avvisi correnti e risolti,selezionare ciascun avviso>>seguire le azioni consigliate.
 ...
 a| 
image:icon_alert_green_checkmark.png["icona di avviso segno di spunta verde"]
 a| 
*Connesso*
...
image:icon_alert_red_critical.png["Icona Avviso rosso critico"] *Critico*: Si è verificata una condizione anomala che ha interrotto le normali operazioni di un nodo o servizio StorageGRID. Devi risolvere immediatamente il problema alla radice. Se il problema non viene risolto, potrebbero verificarsi interruzioni del servizio e perdita di dati.
...
. Per espandere i gruppi di avvisi, seleziona la freccia in basso image:icon_alert_caret_down.png["icona freccia giù"]. Per comprimere i singoli avvisi di un gruppo, seleziona la freccia in alto image:icon_alert_caret_up.png["Icona freccia su"] o il nome del gruppo.
...
. Per ordinare gli avvisi correnti o i gruppi di avvisi, seleziona le frecce su/giù image:icon_alert_sort_column.png["Icona delle frecce di ordinamento"] nell'intestazione di ciascuna colonna.
+
...
.. Per chiudere la finestra a comparsa, seleziona un punto qualsiasi al di fuori della finestra a comparsa.
