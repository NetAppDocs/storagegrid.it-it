...
---
= Distribuisci i nodi di griglia come macchine virtuali per StorageGRID con VMware vSphere
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
Potresti dover rimappare una porta se le policy di rete aziendali limitano l'accesso a una o più porte utilizzate da StorageGRID. Consulta link:../network/index.html["Linee guida per il networking"] per le porte utilizzate da StorageGRID.
