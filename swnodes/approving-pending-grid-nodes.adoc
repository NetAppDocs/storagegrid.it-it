...
---
= Approva i nodi della griglia in sospeso per StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:5_gmi_installer_grid_nodes_pending.gif["Questa immagine viene spiegata dal testo circostante."]
...
.. Torna alla pagina iniziale e seleziona *Avvia installazione*.
...
+
image:7_gmi_installer_grid_nodes_approved.gif["Questa immagine viene spiegata dal testo circostante."]

. Ripetere questi passaggi per ogni nodo griglia in sospeso che si desidera approvare.
+
Devi approvare tutti i nodi che vuoi nella grid. Tuttavia, puoi tornare su questa pagina in qualsiasi momento prima di selezionare *Install* nella pagina Summary. Puoi modificare le proprietà di un nodo della grid approvato selezionando il suo pulsante di opzione e selezionando *Edit*.

. Quando hai finito di approvare i nodi della griglia, seleziona *Avanti*.
