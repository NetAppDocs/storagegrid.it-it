...
---
= Creare file di configurazione del nodo StorageGRID per le distribuzioni Linux
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
| Valore | Designazione 
 ...
 a| 
Indirizzo IPv4 della rete Grid del nodo di amministrazione che si desidera utilizzare per installare il nodo basato su Linux.  Per il ripristino, utilizzare l'IP del nodo di amministrazione primario, se disponibile; in caso contrario, utilizzare l'IP di un nodo di amministrazione non primario.  Se si omette questo parametro, il nodo tenta di individuare un nodo amministrativo primario tramite mDNS.
...
| Valore | Designazione 
 ...
 a| 
DedicatedInterface (Questo è l'unico valore supportato.)
 a| 
Opzionale
...
Per maggiori dettagli sulla clonazione MAC per Linux, vederelink:../swnodes/configuring-host-network.html#considerations-and-recommendations-for-mac-address-cloning["Considerazioni e consigli per la clonazione degli indirizzi MAC"]
 a| 
Best practice
...
| Valore | Designazione 
 ...
 a| 
DedicatedInterface (Questo è solo un valore supportato.)
 a| 
Opzionale
