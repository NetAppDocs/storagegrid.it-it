...
* Le  `If-Match` e  `If-None-Match` intestazioni si comportano in modo diverso a seconda della configurazione di StorageGRID. Fornire queste intestazioni potrebbe restituire un messaggio di errore (501 Not Implemented). Se le  `If-Match` o  `If-None-Match` intestazioni vengono accettate ma non sono funzionanti, potrebbe essere attivato un avviso con istruzioni su come abilitare il comportamento di errore più restrittivo 501 Not Implemented.
