...
NOTE: StorageGRID non supporta l' `x-amz-sdk-checksum-algorithm`intestazione della richiesta.
...
* Gestisci le impostazioni del bucket dal Tenant Manager. Vedi link:../tenant/update-default-retention-settings.html["Aggiorna la conservazione predefinita del blocco oggetti S3"].
...
Devi specificare una delle seguenti intestazioni della richiesta nella richiesta PutObjectLockConfiguration:
...
NOTE: StorageGRID non convalida il contenuto dell'intestazione x-amz-checksum-* per questa operazione.



==== Esempio di richiesta

Questo esempio abilita S3 Object Lock per un bucket (se l'amministratore della griglia ha dato il permesso) e imposta la modalità di conservazione predefinita su COMPLIANCE e il periodo di conservazione predefinito su 6 anni.
...
Quando nuove versioni di oggetti vengono inserite nel bucket, viene applicata la modalità di conservazione predefinita se `x-amz-object-lock-mode` non è specificata. Il periodo di conservazione predefinito viene utilizzato per calcolare la retain-until-date se `x-amz-object-lock-retain-until-date` non è specificato.
...
* Devi includere una delle seguenti intestazioni di richiesta se nella richiesta PutObject è presente una qualsiasi intestazione di richiesta x-amz-object-lock-*:
+
[listing]
----
Content-MD5
...
----
+
Queste intestazioni non sono necessarie per CopyObject o CreateMultipartUpload.

* Se il bucket non ha S3 Object Lock abilitato ed è presente un'intestazione di richiesta x-amz-object-lock-*, viene restituito un errore 400 Bad Request (InvalidRequest).
...
Devi specificare una delle seguenti intestazioni della richiesta:
...
NOTE: StorageGRID non convalida il contenuto dell'intestazione x-amz-checksum-* per queste operazioni.
...
+
L' `x-amz-bypass-governance-retention`intestazione non ha alcun effetto sugli oggetti senza S3 Object Lock.
