...
summary: Il sistema StorageGRID supporta un massimo di 20.000 bucket per ogni account tenant S3. 
---
...
[role="lead"]
Il sistema StorageGRID supporta un massimo di 20.000 bucket per ogni account tenant S3.

Ogni griglia può avere un massimo di 100.000 secchi.

Se più tenant dispongono ciascuno di un massimo di 5.000 bucket, ogni Storage Node nella griglia deve avere un minimo di 64 GB di RAM.
...
Puoi verificare se gli aggiornamenti all'ora dell'ultimo accesso sono abilitati o disabilitati per i singoli bucket. Consulta link:get-bucket-last-access-time-request.html["OTTIENI l'ultimo tempo di accesso a bucket"].
...
*Nota*: i bucket di directory S3 non sono supportati.
...
+
*Nota*: Si verificherà un errore se la tua richiesta CreateBucket utilizza una regione che non è stata definita in StorageGRID.

* Puoi includere l' `x-amz-bucket-object-lock-enabled`intestazione della richiesta per creare un bucket con S3 Object Lock abilitato. Consulta link:../s3/use-s3-api-for-s3-object-lock.html["Utilizzare l'API REST S3 per configurare il blocco oggetti S3"].
+
S3 Object Lock richiede il versioning del bucket, che viene abilitato automaticamente quando crei il bucket con S3 Object Lock abilitato.

+
Puoi abilitare S3 Object Lock quando crei il bucket o in un secondo momento.
...
DeleteBucketLifecycle
 a| 
Elimina la configurazione del ciclo di vita dal bucket. Consulta link:create-s3-lifecycle-configuration.html["Creare la configurazione del ciclo di vita S3"].
...
*Attenzione*: se per questo bucket è impostato un tag di policy ILM non predefinito, sarà presente un  `NTAP-SG-ILM-BUCKET-TAG` tag del bucket con un valore assegnato. Non inviare una richiesta DeleteBucketTagging se è presente un  `NTAP-SG-ILM-BUCKET-TAG` tag del bucket. Invece, invia una richiesta PutBucketTagging con solo il  `NTAP-SG-ILM-BUCKET-TAG` tag e il valore assegnato per rimuovere tutti gli altri tag dal bucket. Non modificare o rimuovere il  `NTAP-SG-ILM-BUCKET-TAG` tag del bucket.
...
(Precedentemente denominato ciclo di vita bucket GET)
 a| 
Restituisce la configurazione del ciclo di vita per il bucket. Consulta link:create-s3-lifecycle-configuration.html["Creare la configurazione del ciclo di vita S3"].
...
GetBucketLocation
 a| 
Restituisce la regione che hai impostato usando l’ `LocationConstraint`elemento nella richiesta CreateBucket. Se la regione del bucket è  `us-east-1` (predefinita), viene restituito un valore vuoto per  `LocationConstraint`.
...
*Attenzione*: se per questo bucket è impostato un tag di policy ILM diverso da quello predefinito, ci sarà un tag del bucket  `NTAP-SG-ILM-BUCKET-TAG` con un valore assegnato. Non modificare o rimuovere questo tag.
...
Consulta link:../s3/use-s3-api-for-s3-object-lock.html["Utilizzare l'API REST S3 per configurare il blocco oggetti S3"].
...
Fai riferimento a link:create-s3-lifecycle-configuration.html["Creare la configurazione del ciclo di vita S3"]. Per capire come l'azione di Expiration nel ciclo di vita di un bucket interagisce con le istruzioni di posizionamento ILM, fai riferimento a link:../ilm/how-ilm-operates-throughout-objects-life.html["Come ILM opera per tutta la vita di un oggetto"].
...
PutBucketPolicy
 a| 
Imposta i criteri associati al bucket. Fai riferimento a link:use-access-policies.html["Utilizza policy di accesso a bucket e gruppi"].
...
* I bucket di destinazione devono essere specificati come URN degli endpoint StorageGRID come specificato in Tenant Manager o nell'API di gestione del tenant. Consulta link:../tenant/configuring-cloudmirror-replication.html["Configurare la replica di CloudMirror"].
+
...
*Attenzione*: se per questo bucket è impostato un tag di policy ILM non predefinito, ci sarà un tag  `NTAP-SG-ILM-BUCKET-TAG` del bucket con un valore assegnato. Assicurati che il tag  `NTAP-SG-ILM-BUCKET-TAG` del bucket sia incluso con il valore assegnato in tutte le richieste PutBucketTagging. Non modificare o rimuovere questo tag.
...
* Se l'amministratore della griglia ha dato il permesso, abiliti S3 Object Lock sui bucket esistenti.


Se il periodo di conservazione predefinito viene modificato, la retain-until-date delle versioni degli oggetti esistenti rimane la stessa e non viene ricalcolata utilizzando il nuovo periodo di conservazione predefinito.

Per informazioni dettagliate, consulta link:../s3/use-s3-api-for-s3-object-lock.html["Utilizzare l'API REST S3 per configurare il blocco oggetti S3"].
