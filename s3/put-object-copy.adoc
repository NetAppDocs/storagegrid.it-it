...
StorageGRID non analizza né interpreta i caratteri UTF-8 preceduti da escape inclusi nel nome o nel valore della chiave dei metadati definiti dall'utente. I caratteri UTF-8 preceduti da escape vengono trattati come caratteri ASCII:
...
* `Content-Type`
* `x-amz-checksum-algorithm`: Specifica `CRC32`, `CRC32C`, `CRC64NVME`, o `SHA256`
+
Quando copi un oggetto, se l'oggetto sorgente ha un  `checksum` o include  `x-amz-checksum-algorithm`, l'oggetto copiato ha un checksum FULL_OBJECT.

+
Se l'oggetto sorgente ha un checksum ma la richiesta non include  `x-amz-checksum-algorithm` o utilizza un valore di checksum non supportato, l'oggetto copiato avrà lo stesso tipo di checksum dell'oggetto sorgente. StorageGRID ignora valori  `x-amz-checksum-algorithm` non validi o non supportati.

+
Se l'oggetto sorgente non ha un checksum e non include `x-amz-checksum-algorithm` oppure utilizza un valore di checksum non supportato, l'oggetto copiato non avrà un checksum.

+

NOTE: Se una richiesta CopyObject non specifica un checksum e l'oggetto sorgente non ha un checksum, StorageGRID non calcola un checksum CRC-64/NVME predefinito per l'oggetto.
