...
---
= Aggiungi un oggetto a un bucket in StorageGRID con la richiesta S3 PutObject
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
** StorageGRID non verifica il  `chunk-signature` rispetto ai dati dei chunk.
** StorageGRID non verifica il valore che fornisci per  `x-amz-decoded-content-length` rispetto all'oggetto.
...
NOTE: Se una richiesta PutObject non specifica un checksum, StorageGRID non calcola un checksum CRC-64/NVME predefinito per un oggetto.
