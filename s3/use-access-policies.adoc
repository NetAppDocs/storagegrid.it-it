...
summary: 'StorageGRID utilizza il linguaggio delle policy di Amazon Web Services (AWS) per consentire ai tenant S3 di controllare l"accesso ai bucket e agli oggetti all"interno di tali bucket.' 
---
...
* I caratteri internazionali, che puoi specificare nella chiave dell'oggetto, devono essere codificati usando JSON UTF-8 o le sequenze di escape JSON \u. La codifica percentuale non è supportata.
+
...
----
* Il valore della risorsa può specificare un bucket che non esiste ancora quando viene creato un criterio di gruppo.
...
== [[use-putoverwriteobject-permission]]Utilizza il permesso PutOverwriteObject
...
* Quando questa autorizzazione non è presente, l'effetto è lo stesso come se fosse impostata l'opzione "Consenti".
...
Operatori IP
 a| 
Corrisponde all'indirizzo IP da cui è stata inviata la richiesta. Può essere utilizzato per operazioni su bucket o oggetti.

*Nota:* Se la richiesta S3 è stata inviata tramite il servizio Load Balancer sui nodi Admin e sui nodi Gateway, questo viene confrontato con l'indirizzo IP a monte del servizio Load Balancer.

*Nota*: Se viene utilizzato un bilanciatore di carico di terze parti non trasparente, il confronto viene effettuato con l'indirizzo IP di tale bilanciatore di carico. Qualsiasi  `X-Forwarded-For` intestazione viene ignorata perché non è possibile accertarne la validità.
...
Risorsa/identità
 a| 
Corrisponde al nome utente del mittente da cui è stata inviata la richiesta. Può essere utilizzato per operazioni su bucket o oggetti.
...
s3:autorizzazioni ListBucketVersions
 a| 
Confronta con il parametro delimitatore specificato in una richiesta ListObjects o ListObjectVersions.
...
s3:PutObjectVersionTagging
 a| 
Richiede che l'oggetto esistente abbia la chiave e il valore del tag specifici.
...
s3:autorizzazioni ListBucketVersions
 a| 
Confronta con il parametro max-keys specificato in una richiesta ListObjects o ListObjectVersions.
...
s3:autorizzazioni ListBucketVersions
 a| 
Si confronta con il parametro prefisso specificato in una richiesta ListObjects o ListObjectVersions, oppure con la chiave specificata per GetObject o HeadObject.
...
s3:PutObjectVersionTagging
 a| 
Richiede una chiave e un valore di tag specifici quando la richiesta dell'oggetto include il tagging.
...
s3:PutObject
 a| 
Confronta con x-amz-copy-source espanso dall'intestazione della richiesta nelle richieste PutObject e UploadPartCopy.
...
Bucket
 a| 
Valido. Gli oggetti sono di proprietà dell'account creatore e la policy del bucket non si applica. L'account creatore deve concedere le autorizzazioni di accesso per l'oggetto utilizzando gli ACL dell'oggetto.
 a| 
...
NOTE: Impostare DeleteObject su DENY in una policy S3 non impedisce a ILM di eliminare gli oggetti quando esiste una regola come "zero copie dopo 30 giorni".
