...
summary: 'Quando un servizio viene arrestato correttamente, viene generato questo messaggio per indicare che è stato richiesto l"arresto e che il servizio ha avviato la sua sequenza di arresto. SYST può essere utilizzato per determinare se è stato richiesto l"arresto, prima che il servizio venga riavviato (a differenza di SYSD, che di solito viene inviato dopo il riavvio del servizio).' 
---
= Messaggio di audit SYST per l'arresto del nodo in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
