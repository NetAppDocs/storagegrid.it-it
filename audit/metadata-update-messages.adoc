...
---
= Messaggi di controllo per gli aggiornamenti dei metadati degli oggetti S3 in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Il client S3 effettua una richiesta (SUPD) per aggiornare i metadati specificati (`x-amz-meta-*`) per l'oggetto S3 (S3KY).

In questo esempio, le intestazioni della richiesta sono incluse nel campo HTRH perché è stato configurato come intestazione del protocollo di audit (*Configuration* > *Monitoring* > *Audit and syslog server*). Vedi link:../monitor/configure-log-management.html["Configurare la gestione dei log e il server syslog esterno"].
